...
def submit_building():
    ...
    사용자가 "내 건물이 목록에 없다"며 도로명주소를 제출하면:
      ...
      2) 도로명→지번 변환(JUSO) → 건축물대장 실시간 검증(is_living_stay)
      3) 생숙으로 확인되면 master_buildings에 편입(source='user_submitted', verified_at=NOW())
         아니면 사유와 함께 거절 (요청 기록에 남김)
    """
    import os as _os
    ...
    try:
        ...
        return fail(f"주소 변환 중 오류가 발생했습니다: {e}")
    if not juso:
    ...
    if not sgg_cd:
        return fail("시군구코드 매칭에 실패했습니다. 주소 표기를 확인해주세요.")
    ...
    try:
        ...
        return fail(f"건축물대장 조회 중 오류가 발생했습니다: {e}")

    if verdict is None:
        return fail(f"건축물대장을 조회하지 못했습니다({reason}). 잠시 후 다시 시도해주세요.")
    if verdict is False:
        return fail(f"생활숙박시설이 아닌 것으로 확인되었습니다 — {reason}")

    building_name = building_name_hint or title["bld_nm"]
    sgg_text_val = bjdong.sgg_text(sgg_cd) or f"{si_do} {sgg_nm}".strip()
    road_addr = title["new_plat_plc"] or title["plat_plc"] or road_address
    ...
    # 여기 도달 = is_living_stay가 생숙으로 확정(위에서 None/False는 이미 반환)
    # → lodging_type='생활'로 태깅해야 기본 '생숙만' 필터에서 숨지 않는다.
    if existing:
        mb_id = existing["id"]
        cur.execute(
            "UPDATE master_buildings SET verified_at=NOW(), lodging_type='생활' WHERE id=%s",
            (mb_id,),
        )
    else:
        cur.execute("""
            INSERT INTO master_buildings
                (building_name, road_address, sgg_text, sgg_cd, umd_nm, jibun, units, source, verified_at, lodging_type)
            VALUES (%s, %s, %s, %s, %s, %s, %s, 'user_submitted', NOW(), '생활') RETURNING id
        """, (building_name, road_addr, sgg_text_val, sgg_cd, umd_nm, jibun_str, title["ho_cnt"]))
    ...
    return jsonify({
        "status": "verified",
        "message": f"'{building_name}'은(는) 생활숙박시설로 확인되어 목록에 추가했습니다.",

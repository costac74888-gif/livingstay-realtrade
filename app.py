...
def get_buildings_geo():
    ...
    if q:
        ...
        params += [f"%{q}%", f"%{q}%", f"%{q}%"]
    if si_do:
        # '서울' vs '서울특별시'처럼 시도 표기 편차 때문에 누락되는 것을 막기 위해,
        # 양쪽 모두 행정접미사(특별자치도/특별자치시/특별시/광역시/도/시)를 떼어낸
        # 코어 이름(예: '서울')으로 정확히 비교한다.
        core = re.sub(r"(특별자치도|특별자치시|특별시|광역시|도|시)$", "", si_do)
        where.append(
            "regexp_replace(split_part(sgg_text, ' ', 1), "

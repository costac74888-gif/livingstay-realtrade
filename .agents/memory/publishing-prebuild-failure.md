...
프런트 번들 사전검사가 앱 모듈을 import하더라도 스키마 초기화·중단 작업 복구·백필 스레드 같은 운영 부팅 동작을 실행하면 안 된다. 접속 불가능한 DB 주소에서도 번들 전체 검사가 끝나는지 확인한다.

**Why:** Publish 빌드 중 테스트 import가 운영 DB DDL을 시작해 deadlock 재시도에 머물렀고, 화면에는 번들 단계가 반복되는 것처럼 보였다.

**How to apply:** 앱 함수를 재사용하는 빌드 검사는 명시적인 부팅 부작용 차단 모드로 실행하고, 배포 전 DB 단절 검사를 회귀 기준으로 유지한다.
...
**Why:** v3 병합 시 zip의 submit/correction이 정확일치를 써서 면/리 생숙이 전부 매칭 실패했음(architect 발견).
**How to apply:** umd_nm 관련 신규 SQL 작성 시 항상 공백제거 후 비교. `find_bjdong_cd`는 내부에서 이미 공백제거하므로 그대로 넘겨도 됨.

...
## find_bjdong_cd 는 공백 무시 매칭이어야 한다
- 호출자마다 umd_nm 표기가 다르다: sync/discover 는 RTMS 원본('봉평면 면온리', 공백O), cleanup/재검증 은 master 저장 정규화값('봉평면면온리', 공백X).
- 예전 로직은 공백 기준 토큰 tail 비교 + `startswith(umd[:2])` 완화매칭 → 공백 없는 입력에서 엉뚱한 상위 동 반환(봉평면=34000 대신 면온리=34025여야 함). 그 결과 면/리 지역 건물이 전부 표제부없음(None)으로 오판됨.
- **Why**: 면/리 지역은 '면+리' 2토큰이 붙어오고, 저장 시 공백이 제거되므로 공백에 의존하면 안 됨.
- **How to apply**: 양쪽 공백 제거 후, 법정동명 뒤 1·2·3토큰을 공백없이 이어붙인 후보와 '정확일치'로 비교. endswith/substring 쓰면 '교동'이 '서교동'에 걸리는 접미사 오매칭 발생 → 토큰조합 정확일치로 회피.

## cleanup 은 지번(parcel)이 아니라 building_name 으로 거래를 지운다
- 한 지번(sgg_cd+jibun)에 여러 건물이 공존한다(예: 대관령면 용산리 130에 용평 콘도 여러 채). 지번만으로 거래 삭제 시 '유지'되는 이웃 건물 거래까지 삭제됨(데이터 손실).
- transactions 에는 매칭된 master building_name 이 그대로 저장되므로 building_name(+sgg_cd+jibun)으로 해당 건물 거래만 정확히 삭제한다.
- umd_nm 은 거래행=RTMS원본(공백O)/master=정규화(공백X)라 삭제 키로 쓰면 안 맞는다.
- **Why**: transactions 에 building FK 가 없고 parcel 이 공유됨 → building_name 이 유일한 건물 단위 연결고리.

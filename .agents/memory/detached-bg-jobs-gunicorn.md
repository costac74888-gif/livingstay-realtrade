...
- Fencing: include a random `run_id` in the status row, pass that exact ID to the detached child, and verify it before and after advisory-lock acquisition. Heartbeat, lock-failure, spawn-failure, and final writes must all use `WHERE run_id = ...`; a child must never adopt whichever ID is currently stored.

...
- **Screenshot tool cannot prove map UI.** The agent's headless screenshot browser runs on a host
  that is NOT in Kakao's registered domain list, so sdk.js 401s there and the map renders **blank/gray
  with no tiles AND no custom overlays** (markers, labels, tooltips all fail to paint) — even when the
  same code works in the user's real preview and the console logs "[MAP] 마커 N개 표시". Do NOT promise
  or attempt a screenshot as proof of any map overlay feature; it will always be gray. Verify instead
  via: no JS console errors + the marker-count log firing (proves loadMapMarkers ran end-to-end), and
  ask the user to confirm visually in their own browser.

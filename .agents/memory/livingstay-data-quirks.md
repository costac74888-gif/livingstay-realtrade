...
## 주간 이메일 숙박업 폴백 집계
- **규칙:** `master_buildings`와 `lodging_registry`를 대량 주소 OR JOIN으로 직접 결합하지 말고, `addr_norm` 키로 각각 읽어 해시 매칭한다.
- **Why:** 마스터에는 `road_norm`/`jibun_norm`이 저장되지 않아 SQL에서 키를 만들며 OR JOIN하면 대량 조인이 되어 주간 이메일 실행 제한을 넘길 수 있다.
- **How to apply:** 이메일처럼 주기성 저빈도 집계는 마스터·신고 데이터를 각각 한 번 읽고 도로명 우선, 지번 보조 키로 permit 번호를 중복 제거해 합산한다.

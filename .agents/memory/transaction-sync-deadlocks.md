...
**How to apply:** 거래·체크포인트·실패큐 정리는 한 월의 같은 commit 경계에 둔다. 전국 최근 거래 수집도 항상 시군구·월 체크포인트를 사용해 실행 제한이나 게시 중단 뒤 처음부터 반복하지 않는다. rollback 시 누적 통계를 원복하고, DB 예외를 내부에서 삼켜 aborted 트랜잭션을 계속 쓰지 않는다. 이메일 같은 외부 부작용은 commit 성공 뒤에만 실행한다.
...
**How to apply:** 새 legacy 수집 진입 경로도 반드시 공통 writer gate 안에서 status claim 전에 종료 설정을 확인한다. 다만 별도 공급자 원천(예: 고캠핑)까지 legacy 종료 게이트로 막지 말고 전용 실행 경로를 분리한다. 최신 manifest와 무회귀 관찰을 종료 조건으로 쓸 때는 관찰 기록·manifest 적용·종료 검증을 별도 DB fence로 직렬화한다.
...
건물 사진의 기본 운영 방식은 전체 사전수집이 아니라 상세 진입 시 브라우저 TourAPI 온디맨드 조회다. 서버의 기존 성공 사진은 그대로 사용하되, 브라우저 조회 결과와 매칭 없음은 공유 DB가 아니라 건물명·주소를 대조하는 만료형 로컬 캐시에만 저장한다. TourAPI 매칭 없음에만 Street View를 fallback하며 Vworld는 공개 중계하지 않는다. 과거 대량 수집 진입점은 직접 실행도 실패 폐쇄한다.

**Why:** 8만여 건을 순회하면 무료 범위를 넘긴다. 익명 브라우저가 보낸 사진을 공유 DB에 저장하면 TourAPI 응답을 서버가 독립 검증할 수 없어 다른 건물 사진으로 공용 캐시를 오염시킬 수 있으므로, 사용자가 로컬 캐시 방식을 승인했다.

**How to apply:** 익명 브라우저 결과를 공유 DB에 쓰지 않고 만료형 로컬 캐시로만 보관한다. 유료 fallback은 로컬 상태만으로 허용하지 말고 서버가 확인할 수 있는 자격과 전역 예산을 모두 적용한다. 브라우저 직접 호출 키는 공개된다는 전제로 별도 브라우저용 키와 공급자 측 origin/quota 제한을 우선한다.

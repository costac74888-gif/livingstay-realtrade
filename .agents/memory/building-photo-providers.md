...
TourAPI 숙박사진은 초기 1회 주소 매칭된 전체 대상을 재개형 배치로 수집하고, 이후 운영관리자 실행은 신규 건물이나 콘텐츠 ID가 바뀐 대상만 증분 수집한다.

**Why:** 숙박 대표사진은 약 2천여 매칭 건물에서 최초 확보 뒤 변동이 드물어, 매 상세 진입 때 외부 연결 상태에 의존하는 것보다 초기 수집 후 변동분만 갱신하는 편이 안정적이라는 운영 결정을 사용자가 승인했다.

**How to apply:** 전국 카탈로그 주소 매칭을 먼저 완료한 다음 콘텐츠 ID 기준으로 대표·다중사진을 저장한다. 완료 상태와 같은 콘텐츠 ID는 다시 수집하지 않고, 신규 매칭·콘텐츠 ID 변경·미완료 체크포인트만 관리자 실행 대상으로 삼는다.
...
Street View는 가장 가까운 파노라마와 고정 방향만 쓰면 저층 벽·상가나 옆 건물이 선택될 수 있다. 건물 양쪽 공식 촬영 지점의 정면 2장을 먼저 비교하고, 점수가 낮을 때만 좌·우 4장을 추가한다.

**Why:** 42층 복합건물에서 31m 최근접 파노라마는 빈 저층 벽만 보였지만, 반대편 42~50m 공식 파노라마는 정면 저층부와 타워를 함께 담았다. 220px `cover` 표시는 4:3 원본의 위아래를 다시 잘랐다.

**How to apply:** 일반 건은 이미지 2회 평가 후 최고점이 0.58 이상이면 표시하고, 그보다 낮은 어려운 건만 좌·우를 더해 최대 6회 평가한다. 확보된 공식 촬영본은 고정 점수로 숨기지 않고 최고 사진을 표시한다. 반복 비용은 긍정·부정 캐시와 호출·동시성 제한으로 막고, 직접 등록과 TourAPI 사진을 우선한다.

최대 6회 평가 도입 뒤 첫 30일은 기본 2회와 추가 4회의 호출·채택 결과를 일별로 계측한다. 추가 이미지 호출이 100회 이상인데 추가 채택률이 5% 미만이면 관리자 액션센터에서 2회 검증 복귀를 검토하도록 경고한다.

**Why:** 오래된 Google 촬영본에서는 반대 방향이 현재 건물처럼 보일 수 있어 추가 호출이 정확도 향상 없이 한도만 소모할 가능성이 있다.

**How to apply:** 사진 캐시 적중은 다시 세지 않고 실제 이미지 평가만 집계한다. 첫 30일 창을 고정해 이후 누적으로 결론이 왜곡되지 않게 하고, 경고가 없으면 현 방식을 유지한다.

Street View 웹 응답 경로에서는 후보마다 Tesseract OCR을 실행하지 않는다. 파노라마 위치·건물 방향·노출 점수로 즉시 선택하고, 무거운 OCR이 필요하면 사용자 요청과 분리된 백그라운드 수집에서만 수행한다.

**Why:** 운영에서 여러 Street View 요청이 겹치자 두 Gunicorn worker가 모두 OCR 자식 프로세스를 실행하며 120초 timeout으로 동시에 종료되어 서버 오류가 발생했다.

**How to apply:** 주변 Metadata 조회는 병렬화하되 네트워크 제한시간을 짧게 둔다. 동기 화면 요청은 외부 호출과 경량 이미지 분석만 수행하고 프로세스 생성 작업을 넣지 않는다.

Street View 후보 수집과 촬영거리 점수는 건물 높이에 따라 달라야 한다. 3층 이하는 가장 가까운 고유 파노라마 2곳을 먼저 평가하고, 중층은 약 26m, 고층은 약 32m 촬영거리를 기준으로 삼는다.

**Why:** 2층 골목 건물에서 Google이 보여주는 정답 정면은 8.3m·9.1m 파노라마였지만, 18m 미만 후보를 버리는 규칙 때문에 17.9m 옆 골목과 26.3m 맞은편 상가만 평가했다.

**How to apply:** 저층은 가까운 필지 정면을 후보 단계부터 보존하고 고층만 먼 촬영점으로 전체 외관을 담는다. 선택 규칙 변경 시 브라우저의 하루 이미지 캐시를 피하도록 내부 사진 URL 버전도 올린다.
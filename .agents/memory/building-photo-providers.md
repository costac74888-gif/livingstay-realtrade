...
Street View 촬영거리 점수는 건물 높이에 따라 달라야 한다. 3층 이하는 약 20m, 4~7층은 약 26m, 그보다 높은 건물은 약 32m를 기준으로 삼는다.

**Why:** 2층 골목 건물에서 좌표는 0.6m 오차로 정확했지만, 고층용 32m 선호 점수가 17.9m의 실제 건물 정면보다 26.3m 맞은편 상가를 높게 선택했다.

**How to apply:** 저층은 가까운 필지 정면을 우선하고 고층만 먼 촬영점으로 전체 외관을 담는다. 선택 규칙 변경 시 브라우저의 하루 이미지 캐시를 피하도록 내부 사진 URL 버전도 올린다.
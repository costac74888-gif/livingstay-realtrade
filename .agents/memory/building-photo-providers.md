...
Street View는 가장 가까운 파노라마와 고정 방향만 쓰면 저층 벽·상가나 옆 건물이 선택될 수 있다. 주변 공식 촬영 지점 최대 3곳과 지점별 3각도를 비교하고, 위치·중앙 노출·OCR 점수를 합산해 최고점을 자동 선택한다.

**Why:** 42층 복합건물에서 31m 최근접 파노라마는 빈 저층 벽만 보였지만, 반대편 42~50m 공식 파노라마는 정면 저층부와 타워를 함께 담았다. 220px `cover` 표시는 4:3 원본의 위아래를 다시 잘랐다.

**How to apply:** 신뢰도가 낮아도 관리자 후보선택 업무를 만들지 않고 사용 가능한 최고점으로 종료한다. 반복 비용은 결과 캐시·호출 제한·동시성 제한으로 막고, 직접 등록과 TourAPI 사진은 항상 Street View보다 우선한다.
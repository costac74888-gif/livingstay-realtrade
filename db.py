...
SCHEMA_VERSION = "2026-09-25-recent-analysis"
...
def _run_init_db():
    ...
    CREATE TABLE IF NOT EXISTS master_buildings (
        ...
        biz_units INTEGER,            -- 레거시 엑셀 스냅샷 참고값(신고율 계산에 사용 금지)
         kakao_promo_copy_count INTEGER NOT NULL DEFAULT 0, -- 관리자 카카오 홍보 문구 복사 횟수
    ...
    CREATE TABLE IF NOT EXISTS annual_tourism_roster_building_evidence (
        ...
        match_status TEXT NOT NULL CHECK (match_status IN ('matched', 'unmatched', 'ambiguous', 'conflict')),
    ...
    cur.execute("""ALTER TABLE annual_tourism_roster_building_evidence
                   DROP CONSTRAINT IF EXISTS annual_tourism_roster_building_evidence_match_status_check""")
    cur.execute("""ALTER TABLE annual_tourism_roster_building_evidence
                   ADD CONSTRAINT annual_tourism_roster_building_evidence_match_status_check
    ...
    """)
    # 숙박 검색순위는 원본 관광 통계와 건물을 느슨하게 연결한다. 건물이 정리돼도
    # 원본 통계는 보존되어야 하므로 삭제 시 FK만 NULL로 만든다.
    cur.execute("""
        ALTER TABLE tourism_stats
        ADD COLUMN IF NOT EXISTS master_building_id INTEGER
            REFERENCES master_buildings(id) ON DELETE SET NULL
    """)
    # ADD COLUMN IF NOT EXISTS does not retrofit REFERENCES when an earlier,
    # incomplete deployment already added the column.
    cur.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1
                FROM pg_constraint
                WHERE conrelid = 'tourism_stats'::regclass
                  AND contype = 'f'
                  AND confrelid = 'master_buildings'::regclass
            ) THEN
                ALTER TABLE tourism_stats
                ADD CONSTRAINT tourism_stats_master_building_id_fkey
                FOREIGN KEY (master_building_id)
                REFERENCES master_buildings(id) ON DELETE SET NULL;
            END IF;
        END $$;
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_tourism_stats_lodging_rank_building
        ON tourism_stats(master_building_id)
        WHERE stat_type = 'lodging_search_rank'
          AND master_building_id IS NOT NULL
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_tourism_stats_lodging_rank_latest
        ON tourism_stats(source_period DESC NULLS LAST, collected_at DESC, source_file DESC)
        WHERE stat_type = 'lodging_search_rank'
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_tourism_stats_lodging_rank_source
        ON tourism_stats(source_file, metric_value, id)
        WHERE stat_type = 'lodging_search_rank'
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_tourism_stats_lodging_rank_building_source
        ON tourism_stats(master_building_id, source_file, metric_value)
        WHERE stat_type = 'lodging_search_rank'
          AND master_building_id IS NOT NULL
    """)
    cur.execute("""
    ...
    """)
    cur.execute("""
    CREATE TABLE IF NOT EXISTS tourism_dong_coords (
        id SERIAL PRIMARY KEY,
        sido_name TEXT NOT NULL,
        sgg_name TEXT NOT NULL,
    ...
    cur.execute(
        "ALTER TABLE master_buildings "
        "ADD COLUMN IF NOT EXISTS kakao_promo_copy_count INTEGER NOT NULL DEFAULT 0"
    )
    ...
    # Administrator notification outbox.  The unique delivery key is also passed
    # to the provider, making retries safe across process crashes and cron overlap.
    cur.execute("""
    CREATE TABLE IF NOT EXISTS admin_notification_deliveries (
        id BIGSERIAL PRIMARY KEY,
        channel TEXT NOT NULL CHECK (channel IN ('immediate', 'daily_digest')),
        idempotency_key TEXT NOT NULL,
        status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'attempting', 'sent', 'failed')),
    ...
    """)
    # Delivery state is intentionally separate from source rows: an email outage
    # must not undo a submitted bug/listing, and stale leases are reclaimable.
    cur.execute("ALTER TABLE admin_notification_deliveries ADD COLUMN IF NOT EXISTS source_kind TEXT")
    cur.execute("ALTER TABLE admin_notification_deliveries ADD COLUMN IF NOT EXISTS source_id BIGINT")
    cur.execute("ALTER TABLE admin_notification_deliveries ADD COLUMN IF NOT EXISTS label TEXT")
    cur.execute("ALTER TABLE admin_notification_deliveries ADD COLUMN IF NOT EXISTS attempting_at TIMESTAMPTZ")
    cur.execute("ALTER TABLE admin_notification_deliveries ADD COLUMN IF NOT EXISTS attempts INTEGER NOT NULL DEFAULT 0")
    cur.execute("ALTER TABLE admin_notification_deliveries ADD COLUMN IF NOT EXISTS next_attempt_at TIMESTAMPTZ NOT NULL DEFAULT NOW()")
    cur.execute("ALTER TABLE admin_notification_deliveries ADD COLUMN IF NOT EXISTS outcome TEXT")
    cur.execute("""
        ALTER TABLE admin_notification_deliveries
        DROP CONSTRAINT IF EXISTS admin_notification_deliveries_status_check
    """)
    cur.execute("""
        ALTER TABLE admin_notification_deliveries
        ADD CONSTRAINT admin_notification_deliveries_status_check
        CHECK (status IN ('pending', 'attempting', 'sent', 'failed'))
    """)
    cur.execute("""CREATE INDEX IF NOT EXISTS idx_admin_delivery_dispatch
                   ON admin_notification_deliveries(channel, status, next_attempt_at)""")
    ...
    cur.execute("""CREATE INDEX IF NOT EXISTS idx_tourism_datalab_stages_expiry
    ...
    # 관리자 영구삭제 전 복구 보관소 — 원본/이력/채팅/사진/찜과 삭제 주체를 함께 보존
    cur.execute("""
        CREATE TABLE IF NOT EXISTS listing_request_deletion_archive (
            id BIGSERIAL PRIMARY KEY,
            listing_request_id INTEGER NOT NULL,
            request_snapshot JSONB NOT NULL,
            related_snapshot JSONB NOT NULL DEFAULT '{}'::jsonb,
            deleted_by_admin_id INTEGER,
            deletion_source VARCHAR(40) NOT NULL,
    ...
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS ix_listing_request_deletion_archive_request
        ON listing_request_deletion_archive(listing_request_id, deleted_at DESC)
    """)

    # Street View 2회→최대 6회 평가의 호출 효율을 일별로 기록한다.
    cur.execute("""
        CREATE TABLE IF NOT EXISTS streetview_evaluation_metrics (
            metric_date DATE PRIMARY KEY DEFAULT CURRENT_DATE,
            evaluations INTEGER NOT NULL DEFAULT 0,
            base_calls INTEGER NOT NULL DEFAULT 0,
            extra_calls INTEGER NOT NULL DEFAULT 0,
            accepted_from_base INTEGER NOT NULL DEFAULT 0,
    ...
    """)
    cur.execute("ALTER TABLE buy_requests ADD COLUMN IF NOT EXISTS area_sqm NUMERIC")
    ...
    """)
    staging_schema_cur.execute("""
    CREATE TABLE IF NOT EXISTS lodging_parallel_comparisons (
        id BIGSERIAL PRIMARY KEY,
        promotion_manifest_id BIGINT NOT NULL
            REFERENCES lodging_promotion_manifests(id) ON DELETE RESTRICT,
        run_id TEXT NOT NULL UNIQUE,
        ...
        major_regression_count INTEGER NOT NULL DEFAULT 0,
        created_at TIMESTAMP NOT NULL DEFAULT NOW()
    )
    """)
    staging_schema_cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_lodging_parallel_comparisons_recent
        ON lodging_parallel_comparisons(
            promotion_manifest_id, created_at DESC, id DESC
        )
    """)
    staging_schema_cur.execute("""
    ...
    # 로그인 회원별 최근 분석 건물. 거래 원장과 별도로 선택 이력만 최대 30개 보관한다.
    cur.execute("""
    CREATE TABLE IF NOT EXISTS user_recent_analysis (
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        building_id INTEGER NOT NULL REFERENCES master_buildings(id) ON DELETE CASCADE,
        last_mode TEXT NOT NULL CHECK (last_mode IN ('property', 'rental', 'operation')),
        analyzed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        PRIMARY KEY (user_id, building_id)
    )
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_user_recent_analysis_recent
        ON user_recent_analysis(user_id, analyzed_at DESC, building_id DESC)
    """)
    ...
    cur.execute(
        "ALTER TABLE user_favorites ADD COLUMN IF NOT EXISTS favorite_increase_alert_enabled BOOLEAN NOT NULL DEFAULT FALSE"
    )
    cur.execute(
        "ALTER TABLE user_favorites ADD COLUMN IF NOT EXISTS nearby_change_alert_enabled BOOLEAN NOT NULL DEFAULT FALSE"
    )
    # 관심단지 저장만으로 새 실거래 이메일을 받되, 이후 건물별 알림 끄기는 보존한다.
    cur.execute(
        "ALTER TABLE user_favorites ADD COLUMN IF NOT EXISTS deal_email_alert_enabled BOOLEAN NOT NULL DEFAULT TRUE"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_user_favorites_deal_email_match "
        "ON user_favorites (address, building_name) WHERE deal_email_alert_enabled = TRUE"
    )
    cur.execute(
    ...
    DO $$ BEGIN
        IF EXISTS (
            ...
            WHERE table_name = 'rone_rental_benchmarks'
              AND column_name = 'source_published_at'
        ) THEN
            UPDATE rone_rental_benchmarks
            SET source_checked_at = COALESCE(source_checked_at, source_published_at, collected_at);
        ELSE
            UPDATE rone_rental_benchmarks
            SET source_checked_at = COALESCE(source_checked_at, collected_at);
        END IF;
    END $$;
    """)
    cur.execute("ALTER TABLE rone_rental_benchmarks ALTER COLUMN source_checked_at SET NOT NULL")
    cur.execute("ALTER TABLE rone_rental_benchmarks DROP COLUMN IF EXISTS source_published_at")
    cur.execute("""
    ...
    CREATE TABLE IF NOT EXISTS presale_applications (
        id BIGSERIAL PRIMARY KEY,
        master_building_id INTEGER NOT NULL REFERENCES master_buildings(id) ON DELETE RESTRICT,
        ...
        title TEXT NOT NULL,
        project_status TEXT NOT NULL CHECK (project_status IN ('presale','scheduled','sold_out')),
        ...
        receipt_notification_status TEXT NOT NULL DEFAULT 'pending'
            CHECK (receipt_notification_status IN ('pending','sent','failed')),
        decision_notification_status TEXT NOT NULL DEFAULT 'pending'
            CHECK (decision_notification_status IN ('pending','sent','failed')),
    ...
    CREATE TABLE IF NOT EXISTS presale_audit_log (
        ...
        application_id BIGINT REFERENCES presale_applications(id) ON DELETE SET NULL,
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_presale_applications_review ON presale_applications(status, created_at DESC)")
    cur.execute("""CREATE UNIQUE INDEX IF NOT EXISTS uq_presale_applications_active_building
                   ON presale_applications(master_building_id)
                   WHERE status IN ('submitted','reviewing')""")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_presale_audit_project ON presale_audit_log(project_id, created_at DESC)")
    cur.execute("ALTER TABLE presale_audit_log ADD COLUMN IF NOT EXISTS application_id BIGINT REFERENCES presale_applications(id) ON DELETE SET NULL")
    cur.execute("ALTER TABLE presale_applications ADD COLUMN IF NOT EXISTS receipt_notification_status TEXT NOT NULL DEFAULT 'pending'")
    cur.execute("ALTER TABLE presale_applications ADD COLUMN IF NOT EXISTS decision_notification_status TEXT NOT NULL DEFAULT 'pending'")
    ...
    # Replit Publish가 standalone enum CHECK의 pg_get_constraintdef(= ANY(ARRAY))
    # 괄호를 잘못 변환하므로 applyhome 상태는 애플리케이션 allowlist로 검증한다.

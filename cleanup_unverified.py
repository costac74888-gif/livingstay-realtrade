    ...
    for row in targets:
        ...
        if verdict is True:
            ...
            if not dry_run:
                ...
                # ⚠ 같은 지번(parcel)에 여러 건물이 공존하므로(예: 대관령면 용산리 130의 용평 콘도들)
                #    지번만으로 지우면 '유지'되는 이웃 건물의 거래까지 지워진다.
                #    거래행에는 매칭된 master building_name이 그대로 박혀 있으므로
                #    building_name(+시군구+지번)으로 이 건물의 거래만 정확히 지운다.
                #    (umd_nm은 거래행=RTMS원본(공백O)/master=정규화(공백X)라 키로 쓰면 안 맞음)
                cur.execute("""
                    DELETE FROM transactions
                    WHERE building_name=%s AND sgg_cd=%s AND jibun=%s
                """, (row["building_name"], row["sgg_cd"], row["jibun"]))

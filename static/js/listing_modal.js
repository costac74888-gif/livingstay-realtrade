(function () {
  ...
  window.openListingRequestModal = function (buildingId, buildingName, options) {
    ...
    overlay.innerHTML =
      '<div role="dialog" aria-modal="true" aria-labelledby="lrTitle" style="width:100%;max-width:520px;max-height:calc(100vh - 28px);overflow:auto;background:#fff;border-radius:16px;box-shadow:0 18px 50px rgba(0,0,0,.28);">' +
        ...
        '<form id="lrForm" style="padding:16px 18px 20px;">' +
          ...
          '<section id="lrPhoneSection" style="margin-bottom:17px;"><div style="font-size:12px;font-weight:800;color:var(--ink);margin-bottom:7px;">연락처</div><input id="lrPhone" type="tel" inputmode="tel" placeholder="010-1234-5678" value="' + esc(prefill.contact_phone || "") + '" style="' + inputStyle() + '"><div id="lrPhoneHelp" style="font-size:11.5px;color:var(--ink-soft);margin-top:5px;"></div></section>' +
    ...
    function updateMode() {
      ...
      $("#lrPhoneSection").style.display = dealMode === "broker" ? "block" : "none";
    }
    ...
    $("#lrPhone").addEventListener("input", function () {
      var digits = this.value.replace(/\D/g, "").slice(0, 11);
      this.value = digits.length > 7 ? digits.slice(0, 3) + "-" + digits.slice(3, 7) + "-" + digits.slice(7) :
    ...
    form.addEventListener("submit", function (event) {
      ...
      if (isEdit) body.contact_phone = phone;
      else { body.master_building_id = parseInt(buildingId, 10); body.deal_mode = dealMode; body.contact_phone = phone; }

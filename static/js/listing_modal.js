  ...
  window.openListingRequestModal = function (buildingId, buildingName, options) {
      ...
      '<div role="dialog" aria-modal="true" aria-labelledby="lrTitle" style="width:100%;max-width:520px;max-height:calc(100vh - 28px);overflow:auto;background:#fff;border-radius:16px;box-shadow:0 18px 50px rgba(0,0,0,.28);">' +
        ...
        '<form id="lrForm" style="padding:16px 18px 20px;">' +
          ...
          '<div id="lrModeHelp" style="font-size:11.5px;color:var(--ink-soft);margin-top:6px;"></div></section>' +
           ...
           '<div id="lrPriceSale" class="lr-field"><label class="lr-field-label" for="lrSalePrice">매매가</label><input id="lrSalePrice" type="number" min="1" inputmode="numeric" placeholder="만원" value="' + esc(prefill.price_krw || "") + '" style="' + inputStyle() + '"></div>' +
          ...
          '<div style="display:flex;gap:7px;"><input id="lrYieldDeposit" type="number" min="1" inputmode="numeric" placeholder="보증금 (만원)" value="' + esc(prefill.deposit_krw || "") + '" style="' + inputStyle("flex:1;") + '"><input id="lrYieldRent" type="number" min="1" inputmode="numeric" placeholder="월 임대료 (만원)" value="' + esc(prefill.yield_rent_krw || "") + '" style="' + inputStyle("flex:1;") + '"></div><div id="lrYieldResult" style="font-size:11.5px;color:var(--brass,#b4863f);margin-top:6px;"></div></section>' +
             ...
             '<section id="lrUrgentSection" style="display:none;margin-bottom:17px;padding:12px 13px;border:1px solid #E8B6A5;border-radius:9px;background:#FFF7F3;">' +
             '<label for="lrUrgentSale" style="display:flex;align-items:flex-start;gap:9px;cursor:pointer;"><input id="lrUrgentSale" type="checkbox"' + (prefill.is_urgent ? " checked" : "") + ' style="width:18px;height:18px;margin:1px 0 0;accent-color:#C85A36;flex:0 0 auto;"><span><strong style="display:block;color:#A23E20;font-size:13.5px;">🔥 급매로 등록</strong><span id="lrUrgentHelp" style="display:block;margin-top:3px;color:#6B4A40;font-size:11.5px;line-height:1.5;">체크하면 전체공개 후 직거래 매물목록에 급매로 표시됩니다. 체크하지 않아도 최신 실거래가보다 낮으면 자동으로 급매 표시됩니다.</span></span></label>' +
             '</section>' +
    ...
    form.addEventListener("submit", function (event) {
      ...
      var body = {
        ...
        is_urgent: isWhole && dealMode === "direct" && dealType === "매매" && $("#lrUrgentSale").checked,

...
assert.match(css, /\.presale-table \.presale-share\{color:var\(--ink\);font-weight:800\}/);
assert.match(css, /\.presale-table tfoot th:last-child\{color:var\(--ink\)\}/);
...
assert.match(apply, /params\.get\("building_id"\)\|\|legacy/);
assert.match(apply, /id="presaleBuildingSearch"/);
assert.match(apply, /\/api\/buildings\/search\?q=/);
assert.match(apply, /\/api\/buildings\/\$\{encodeURIComponent\(buildingId\)\}\/brief/);
assert.match(apply, /if\(!r\.ok\)throw Error\(\)/);
assert.match(apply, /if\(!item\)\{buildingId=null;building=null;/);

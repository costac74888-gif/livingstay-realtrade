...
- 관리자 "데이터 동기화" 페이지(`/admin` datasync 메뉴): `scheduled_sync.py`가 매일 02시에 실거래·건축물/인허가·숙박·농어촌민박/한옥·중개/상가 API를 순차 실행하고 `scheduled_sync_status`에 전체/단계별 상태를 저장한다. 건축HUB와 인허가는 공유 쿼터 보호를 위해 월·수·금/화·목·토로 교대한다. 화면은 자동 배치 상태를 먼저 보여주고 기존 개별 실행 도구는 접힌 "수동 보완·재처리" 영역에 둔다. 실패/중단 단계는 체크포인트를 보존해 재시도하며, 이미 성공한 단계는 반복하지 않는다. 좌표·표제부 백필, 과거 실거래 백필, 병합·프리워밍은 정기 배치 밖의 유지보수 작업이다.
...
- **운영 확인 완료 기능 보존**: 운영 사이트의 건물 상세에서 건물 사진 표시가 정상 동작하는 것을 확인함. 건물사진 조회·업로드·우선순위·삭제 관련 코드는 임의로 삭제하거나 되돌리지 않음.
